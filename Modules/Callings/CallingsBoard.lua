...
function WQT_CallingsBoardMixin:UpdateVisibility()
	if (not WorldQuestTab.settings.general.sl_callingsBoard) then
...
function WQT_CallingsBoardDisplayMixin:OnEnter()
	...
	if (self.calling.isLockedToday) then 
		...
		if (self.calling:IsActive()) then
			WQT_Utils:ShowQuestTooltip(self, self.questInfo, WorldQuestTab.Variables["TOOLTIP_STYLES"].callingActive);
		else
			WQT_Utils:ShowQuestTooltip(self, self.questInfo, WorldQuestTab.Variables["TOOLTIP_STYLES"].callingAvailable);
		end
